...
def process_tables(db_path, pg_path, multiprocessing):
    ...
    toc_map = parse_toc_file(db1, pg_path)
...
def parse_toc_file(db, pg_path):
    ...
    for line in data:
        ...
        table = comps2[6]
        dict = {"table":table, "rel":0}
        if table in ignore_tables:
            dict["rel"] = -1
        elif "_" in table:
            dict["rel"] = 1
            if table in to_one_tables:
                dict["rel"] = 2
...
def process_table_wrapper(dict):
    process_table(dict["db1"], dict["gz_filename1"], dict["db2"], dict["gz_filename2"], dict["tmp"], dict["out"], dict["table"])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--db_path', action='store', help="Verzeichnis wo beide datenbanken als  db1/  und  /db2/  liegen")
